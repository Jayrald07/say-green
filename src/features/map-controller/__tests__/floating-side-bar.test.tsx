...
import { act, findByRole, findByTestId, render, screen, waitFor } from "@testing-library/react";
import userEvent from "@testing-library/user-event";
import "@testing-library/jest-dom";
import MapController from "../components/floating-side-bar";
import MapControllerContextProvider from "@app/context/map-controller-context/components";
import Authorization from "@app/components/authorization";
import { getCurrentUser } from "aws-amplify/auth";

describe("tests map controller only", () => {
  const { getByTestId } = render(
    <MapControllerContextProvider>
      <MapController />
    </MapControllerContextProvider>
  );
